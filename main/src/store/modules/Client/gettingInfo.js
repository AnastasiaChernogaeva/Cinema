  ...
  actions: {
    ...
    async buyTickets({ commit }, payload) {
      try {
        ...
        let key;
        keys.forEach((kkey) => {
          if (data[kkey].email.toLowerCase() === email.toLowerCase()) {
            key = kkey;
          }
        });
        if (key === undefined) {
          ...
          const { data: inData } = await axios.post(`/${payload.rType}.json`, {
            ...payload.info,
            uid: key,
          });
        }
    ...
    async resetPassword({ commit }, payload) {
      try {
        const {
          data: inData,
        } = await axios.patch(`/${payload.rType}/${payload.id}.json`, {
          ...payload.info,
